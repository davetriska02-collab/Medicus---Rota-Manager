  ...
  render(root, ctx) {
    ...
    root.innerHTML = `
      ...
      <div class="card">
        ...
        </table>` : '<div class="muted">Nothing rostered in the next two weeks.</div>'}
      </div>
      ...
      <div class="card">
        ...
        ${myLeave.length ? `<table class="mt8">
          ...
          <tbody>${myLeave.map((l) => `<tr><td>${esc((leaveTypeById(l.type) || {}).name || l.type)}</td>
            ...
            <td><span class="pill ${esc(l.status)}">${esc(l.status)}</span></td></tr>`).join('')}</tbody>
        </table>` : ''}
      </div>
      ...
      <div class="card">
        ...
        <div class="toolbar">
          <select id="sw-mine" style="max-width:260px">
            ${swappable(meStaff.id).map((e) => `<option value="${esc(e.id)}">${esc(entryLabel(e))}</option>`).join('')}
          </select>
          <span>⇄</span>
        ...
        ${mySwaps.length ? `<table class="mt8">
          ...
          <tbody>${mySwaps.map((s) => {
            ...
            const other = state.staff.find((p) => b && p.id === (s.status === 'approved' ? a && a.staffId : b.staffId));
    ...
    root.querySelector('#myics').onclick = () => {
      const slug = meStaff.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      download(`rota-${slug}.ics`, staffICS({ person: meStaff, entries: state.entries, rooms: state.rooms || [] }), 'text/calendar');
      ctx.toast('Calendar exported');
    };

    root.querySelector('#ml-submit').onclick = async () => {
      const req = {
        id: uid(), staffId: meStaff.id,
      ...
      if (!req.startDate || !req.endDate || req.endDate < req.startDate) { ctx.toast('Check the dates'); return; }
      ...
      await ctx.log(`${meStaff.name} requested ${req.type} leave ${req.startDate} – ${req.endDate} (${req.sessions} sessions)`);
      ctx.toast(`Request submitted — ${check.warnings.length ? `${check.warnings.length} clash warning(s) for the manager` : 'no clashes'}`);
      ctx.rerender();
    };
    ...
    const fillTheirs = () => {
      theirsSel.innerHTML = swappable(whoSel.value)
        .map((e) => `<option value="${esc(e.id)}">${esc(entryLabel(e))}</option>`).join('') ||
        '<option value="">no upcoming sessions</option>';
    };
    if (whoSel) { whoSel.onchange = fillTheirs; fillTheirs(); }
    ...
    if (swBtn) swBtn.onclick = async () => {
      ...
      state.swaps.push({
        id: uid(), entryAId: entryA.id, entryBId: entryB.id,
        proposedBy: meStaff.id, status: 'requested', createdAt: new Date().toISOString()
      });
      await ctx.persist('swaps');
      await ctx.log(`${meStaff.name} proposed a swap: ${entryA.date} ${entryA.period.toUpperCase()} ⇄ ${entryB.date} ${entryB.period.toUpperCase()}`);
