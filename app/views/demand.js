...
import { mergePull, effective, weekdayProfile, demandToSessions, compareWeek, windowDates } from '../../engine/demand.js';
...
export default {
  render(root, ctx) {
    ...
    const comparison = compareWeek({
      dates: weekDates(weekStart), entries: state.entries, staff: state.staff,
      leaveList: state.leave, settings: s, required
    });
    ...
    root.innerHTML = `
      ...
      <div class="card">
        ...
        ${haveData ? `<table>
          ...
          <tbody>
            ${s.openDays.filter((dk) => profile[dk]).map((dk) => {
              const p = profile[dk];
              const r = required[dk] || {};
        ...
        </table>` : '<div class="muted">Pull data to build the profile.</div>'}
      </div>
      ...
      <div class="card">
        ...
        </table>` : '<div class="muted">Needs a demand profile and rostered sessions for the selected week.</div>'}
      </div>
      ...
      <div class="card">
        ...
        ${histDates.length > 10 ? `<button id="d-showall" class="small" style="margin-top:8px">${showAll ? 'Show last 10' : `Show all ${histDates.length} days`}</button>` : ''}` : '<div class="muted">No history yet.</div>'}
      </div>
    `;
    ...
    if (pullBtn) pullBtn.onclick = async () => {
      ...
      try {
        const [appts, tasks] = await Promise.all([
          ...
          d.includeTasks ? fetchTaskCounts(s.practiceCode, dates[0], dates[dates.length - 1]) : Promise.resolve({ byDate: {}, errors: [] })
        ]);
    ...
    root.querySelectorAll('[data-dfield]').forEach((input) => {
      input.onchange = async () => {
        ...
        if (value === (day[field] ?? 0)) delete day[manualKey]; // matches pulled: no override needed
