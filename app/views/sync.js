...
export default {
  render(root, ctx) {
    ...
    const unknownNames = [...new Set(allFindings.filter((f) => f.kind === 'unknown-clinician').map((f) => f.staffName))];
    ...
    root.innerHTML = `
      ...
      ${results ? `
        <div class="cards">
          ${['missing-clinic', 'ghost-clinic', 'unplanned-clinic', 'unknown-clinician', 'ok'].map((k) => `
        ...
        ${results.map((r) => `
          <div class="card">
            ...
            ${r.findings.filter((f) => f.kind !== 'ok').map((f) => `
            ...
            ${r.findings.some((f) => f.kind === 'ok') ? `
              <details style="margin-top:8px"><summary class="sub">${r.findings.filter((f) => f.kind === 'ok').length} session(s) in sync</summary>
                ${r.findings.filter((f) => f.kind === 'ok').map((f) => `<div class="warn"><span class="sev ok">ok</span><span>${esc(f.message)}</span></div>`).join('')}
              </details>` : ''}
          </div>
        `).join('')}
    ...
    root.querySelector('#run').onclick = async () => {
      ...
      try {
        ...
        state.ui.syncErrors = errs.length ? [...errs, 'Tip: you must be signed in to Medicus in this browser profile.'] : [];
      } catch (err) {
    ...
    root.querySelector('#demo').onclick = () => {
      const dates = weekDates(state.weekMonday).filter((d) => state.settings.openDays.includes(dayKey(d))).slice(0, 2);
      ...
      btn.onclick = async () => {
        ...
        if (state.ui.syncResults) {
          state.ui.syncResults = state.ui.syncResults.map((r) => ({
            ...r,
            findings: diffDay({
              date: r.date, medicusRows: r.medicusRows,
              rotaEntries: state.entries.filter((e) => e.date === r.date),
              staff: state.staff, leaveList: state.leave
            })
          }));
        }
        ctx.rerender();
