...
export function staffLabel(person) {
  ...
  const extra = person.employmentType === 'registrar' ? (person.registrarStage || 'GPST') : (role ? role.name : person.role);
...
export function warnHTML(warnings) {
  ...
  return warnings
    .map((w) => `<div class="warn"><span class="sev ${esc(w.severity)}">${esc(w.severity)}</span><span>${esc(w.message)}</span></div>`)
    .join('');
}
