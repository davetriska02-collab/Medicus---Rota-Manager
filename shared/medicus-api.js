...
export async function fetchTaskCounts(practiceCode, startISO, endISO) {
  const results = await Promise.allSettled(TASK_TYPES.map(async (type) => {
    const url = `${apiBase(practiceCode)}/tasks/data/${type}/task-list` +
  ...
  for (const r of results) {
    if (r.status !== 'fulfilled') { errors.push(r.reason instanceof Error ? r.reason.message : String(r.reason)); continue; }
