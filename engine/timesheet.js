...
export function timesheetCSV({ startDate, endDate, staff, entries, leaveList }) {
  ...
  const header = [
    'Name', 'Role', 'Employment', 'Site', 'Contracted/wk',
    ...SESSION_TYPES.map((t) => t.short),
    'Total sessions', 'Total hours', 'Covered by locum',
  ...
  const lines = [
    ...
    ...rows.map((r) => [
      r.name, r.role, r.employment, r.site, r.contracted,
      ...SESSION_TYPES.map((t) => r.byType[t.id] || 0),
      r.totalSessions, r.totalHours, r.coveredSessions,
