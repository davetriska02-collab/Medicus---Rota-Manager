...
export function solveRota({
  ...
  for (const entry of horizonEntries) {
    ...
    const allowed = isVtsForced
      ? ['tutorial']
      : allowedTypes(originalType, person || {}, eligible, rt);
  ...
  for (const person of staff) {
    ...
    histDutyCount[person.id] = historyEntries.filter(
      (e) => e.staffId === person.id && e.typeId === 'duty' &&
             e.status !== 'cancelled' && e.status !== 'vacancy'
    ).length;
  }
  ...
  function score(currentTypes) {
    ...
    return dutyGapScore + vtsScore + fairnessScore + sameDayScore + weeklyCapScore + locumDutyScore + preferenceScore + churnScore;
  }

  // Score breakdown for final reporting
  function scoreBreakdown(currentTypes) {
    let dutyGap = 0, vts = 0, fairness = 0, sameDay = 0,
        weeklyCap = 0, locumDuty = 0, preference = 0, churn = 0;
    ...
    if (move < 0.40) {
      ...
      if (openSlots.length > 0) {
        ...
        const nonDutyElig = slotEntries.filter((e) => {
          const meta = entryMeta.get(e.id);
          return meta && meta.eligible && meta.allowed.includes('duty') &&
                 (currentTypes.get(e.id) || e.typeId) !== 'duty';
        });
        if (dutyOnes.length > 0 && nonDutyElig.length > 0) {
          ...
          const revertTo = fromMeta && fromMeta.allowed.includes(fromMeta.revertType)
            ? fromMeta.revertType : (fromMeta ? fromMeta.originalType : 'surgery');
          changed = {
            entryId: fromEntry.id, newType: revertTo,
            entryId2: toEntry.id, newType2: 'duty'
          };
        }
      }
    } else {
