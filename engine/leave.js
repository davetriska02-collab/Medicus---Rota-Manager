...
export function leaveBalance(staffMember, leaveList, type, asOfISO) {
  ...
  const used = leaveList
    .filter((l) => l.staffId === staffMember.id && l.type === type && l.status === 'approved' && leaveYearStart(l.startDate) === yearStart)
...
export function checkLeaveRequest(req, { staff, leaveList, entries, settings }) {
  ...
  if (lt && lt.counted) {
    const bal = leaveBalance(person, leaveList.filter((l) => l.id !== req.id), req.type, req.startDate);
  ...
  for (const date of datesInRange(req.startDate, req.endDate)) {
    for (const period of periodsFor(settings)) {
      ...
      if (entry && entry.typeId === 'duty') {
        warnings.push({ severity: 'high', message: `${person.name} is the duty doctor on ${date} ${period.toUpperCase()} — cover must be reassigned` });
      }
    }
  }
...
export function sfeReimbursementFlags(leaveList, staff, asOfISO = todayISO()) {
  return leaveList
    ...
    .map((l) => {
      ...
      const person = staff.find((s) => s.id === l.staffId);
      return { leaveId: l.id, name: person ? person.name : '?', days, eligible: days > 14, nearing: days >= 10 && days <= 14 };
    })
