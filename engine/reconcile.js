...
export function diffDay({ date, medicusRows, rotaEntries, staff, leaveList }) {
  ...
  for (const e of rotaEntries) {
    ...
    if (!row || !row[e.period].hasSession) {
      findings.push({
        kind: 'missing-clinic', severity: 'high', date, period: e.period, staffName: person.name,
        message: `${person.name} is rostered for ${t.name} (${e.period.toUpperCase()}) but no clinic is built in Medicus — lost capacity`
      });
    } else {
      findings.push({
        kind: 'ok', severity: 'ok', date, period: e.period, staffName: person.name,
        booked: row[e.period].booked, slots: row[e.period].slots,
  ...
  for (const row of medicusRows) {
    ...
    for (const period of ['am', 'pm']) {
      ...
      if (leave) {
        findings.push({
          kind: 'ghost-clinic', severity: 'high', date, period, staffName: person.name,
      ...
      const hasRota = rotaEntries.some((e) => {
        const t = typeById(e.typeId);
        return e.staffId === person.id && e.period === period && t && t.buildsClinic &&
          (e.status === 'planned' || e.status === 'confirmed' || e.status === 'covered');
      });
      if (!hasRota) {
        findings.push({
          kind: 'unplanned-clinic', severity: 'medium', date, period, staffName: person.name,
          booked: row[period].booked,
  ...
  for (const row of medicusRows) {
    ...
    findings.push({
      kind: 'unknown-clinician', severity: 'info', date, staffName: row.name,
