...
export function buildEvidenceReport({ startMonday, weeks, staff, entries, leave, rooms, settings, audit = [], generatedBy = '' }) {
  const weekRows = [];
  for (let w = 0; w < weeks; w++) {
    ...
    const openSlots = dates.filter((d) => settings.openDays.includes(dayKey(d)) && !(settings.bankHolidays || []).includes(d)).length * 2;
    weekRows.push({
      monday,
  ...
  const leaveInPeriod = leave.filter((l) => l.status === 'approved' && l.startDate <= endDate && l.endDate >= startMonday);
  const leaveByType = {};
...
<style>
  ...
  <table>
    ...
    ${Object.entries(leaveByType).map(([t, n]) =>
      `<tr><td>${esc((leaveTypeById(t) || { name: t }).name)}</td><td>${n}</td></tr>`).join('') || '<tr><td colspan="2">None</td></tr>'}
  </table>
  ${bradford.length ? `
  ...
  <table>
    ...
    ${audit.slice(-30).reverse().map((a) =>
      `<tr><td>${esc(new Date(a.at).toLocaleString('en-GB'))}</td><td>${esc(a.by || '—')}</td><td>${esc(a.summary)}</td></tr>`).join('')}
  </table>` : ''}
