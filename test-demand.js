...
let required = demandToSessions({ mon: { am: 50, pm: 50 } }, { ...settings, apptsPerSurgerySession: 15, demand: { ...settings.demand, bufferPct: 20 } });
assert.equal(required.mon.am, 4); // 60/15 exact
required = demandToSessions({ mon: { am: 51, pm: 0 } }, { ...settings, apptsPerSurgerySession: 15, demand: { ...settings.demand, bufferPct: 20 } });
...
const rows2 = compareWeek({
  ...
  staff: [gp, nurse], leaveList: [], settings,
