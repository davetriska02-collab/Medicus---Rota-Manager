...
import { solveRota, DEFAULT_WEIGHTS } from './engine/solver.js';
import { newStaff, DEFAULT_SETTINGS } from './shared/model.js';
import { weekDates, addDays } from './shared/time.js';
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const reg = newStaff({ id: uid(), name: 'Dr Reg', employmentType: 'registrar', dutyEligible: false, contractedSessions: 8 });
  ...
  const res = solveRota({
    dates: WEEK, entries, staff: [gp, nurse, reg],
    leaveList: [], settings: MON_ONLY,
  ...
  const badDuty = res.changes.filter(
    (c) => (c.staffId === nurse.id || c.staffId === reg.id) && c.to === 'duty'
  );
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const res = solveRota({
    dates: dates2, entries, staff: [gpA, gpB],
    leaveList: [], settings: settings4,
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const res = solveRota({
    dates: dates3d, entries, staff: [gpBig, gpSmall],
    leaveList: [], settings: settings3d,
  ...
  assert.ok(bigDuty > smallDuty, `Test 3b: 8-session GP (${bigDuty}) should have more duty than 4-session GP (${smallDuty})`);
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const res = solveRota({
    dates: WEEK, entries, staff: [gpX, gpY, gpZ],
    leaveList: [], settings: MON_ONLY,
...
// ────────────────────────────────────────────────────────────────────────────
{
  const partner = newStaff({ id: uid(), name: 'Dr Partner', employmentType: 'partner', supervisor: true, dutyEligible: true, contractedSessions: 8 });
  const reg = newStaff({ id: uid(), name: 'Dr Reg', employmentType: 'registrar', dutyEligible: false, contractedSessions: 8, vtsDay: 'mon-am' });
  ...
  const res = solveRota({
    dates: WEEK, entries, staff: [partner, reg],
    leaveList: [], settings: MON_ONLY,
    options: { seed: 1, iterations: 4000 }
  });
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const resSameDay = solveRota({
    dates: WEEK, entries: entriesSameDay, staff: [gpA, gpB],
    leaveList: [], settings: MON_ONLY,
  ...
  const entriesCap = [
    mkEntry(uid(), gpCap.id, MON, 'am', 'surgery'),
    mkEntry(uid(), gpCap.id, MON, 'pm', 'surgery'),
  ];

  const resCap = solveRota({
    dates: WEEK, entries: entriesCap, staff: [gpCap],
    leaveList: [], settings: MON_ONLY,
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const gpAvoider = newStaff({ id: uid(), name: 'Dr Prev', dutyEligible: true, contractedSessions: 8, avoidDuty: ['mon-am', 'mon-pm'] });
  ...
  const res = solveRota({
    dates: WEEK, entries, staff: [gpAvoider, gpOther],
    leaveList: [], settings: MON_ONLY,
    historyEntries: [
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const res = solveRota({
    dates: WEEK, entries, staff: [gp],
    leaveList: [], settings: bhSettings,
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const run = (seed) => solveRota({
    dates: WEEK, entries, staff: [gp1, gp2],
    leaveList: [], settings: MON_ONLY,
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  const entries = [
    mkEntry(uid(), nurse.id, MON, 'am', 'surgery'),
    mkEntry(uid(), nurse.id, MON, 'pm', 'surgery'),
  ];

  const res = solveRota({
    dates: WEEK, entries, staff: [nurse],
    leaveList: [], settings: MON_ONLY,
    options: { seed: 1, iterations: 2000 }
  });
