...
import { inferRooms, fillRooms } from './engine/room-infer.js';
import { newStaff, typeById } from './shared/model.js';
...
const rowsByDate = {
  '2026-06-01': [row('Dr Alice', cell(true, 3, 3), off), row('Dr Bob', cell(true, 2, 2), off), row('Dr Dave', cell(true, 4, 0), off)],
  '2026-06-02': [row('Dr Alice', cell(true, 3, 3), off), row('Dr Carol', off, cell(true, 1, 1)), row('Dr Eve', off, cell(true, 1, 1))]
};
...
const oneRoom = fillRooms({
  ...
  staff: [alice, bob], rooms: [{ id: 'r1', name: 'Room 1' }], typeById
});
